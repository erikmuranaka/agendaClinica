...
from .models import planoSaude as dadosdb
from .forms import planoSaudeForm as form
...
def listar(request):
    ...
    return render(request, 'listar_plano.html', {'list_dados': obj_dados, 'ativo': ativo_filter})
...
def pesquisar(request):
    ...
    if query:
        obj_dados = dadosdb.objects.filter(
             models.Q(nome__icontains=query) | models.Q(cnpj__icontains=query)
            )
    else:
        obj_dados = dadosdb.objects.filter()
...
def alterar(request, id):
    ...
    return render(request, 'alterar_plano.html', {'form': formDados})
